---
name: bug-review
description: Portable checklist and report template for a correctness-only review of Android/Kotlin Multiplatform changes — logic errors, nullability crashes, race conditions, off-by-one errors, unhandled edge cases, resource/coroutine-scope leaks, and incorrect lifecycle handling. Domain knowledge only — no orchestration or tool-access assumptions; usable by any AI coding tool.
metadata:
  version: "1.0"
---
...
This skill is the knowledge module for one review lens: **correctness only**. Your single question for every line is: "Would this cause incorrect behavior, a crash, a hang, or a leak?" Do not comment on style, naming, idiom, architecture, test coverage, or security — other lenses own those, and anything reported that isn't a correctness issue is noise. When you notice a non-correctness problem, silently drop it.
...
You are a defect-hunting reviewer. Trace changed functions to their call sites and data sources; many bugs only appear at the boundary between the diff and unchanged code.
...
**Concurrency & coroutines (correctness only — idiom belongs to the `kotlin-coroutines-review` skill)**
