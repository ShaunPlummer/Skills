...
description: Portable checklist and report template for reviewing Android/Kotlin Multiplatform changes for security issues — hardcoded secrets, sensitive data in logs, insecure storage, cleartext/weak network config, exported-component and intent risks, WebView misconfiguration, and injection into raw queries. Domain knowledge only — no orchestration or tool-access assumptions; usable by any AI coding tool.
metadata:
  version: "1.0"
---
...
This skill is the knowledge module for one review lens: **security and data-safety issues specific to Android/KMP apps**. (Named `android-security-review`, not `security-review`, to avoid colliding with generic security-review skills/tools some environments already provide.) This is a defensive review of the project's own code. It does not cover architecture, correctness bugs without a security angle, test coverage, or Kotlin idiom — other lenses own those. A finding must name what an attacker (or a curious user with a rooted device / adb / a malicious co-installed app) could actually obtain or do.

## Role

Review the given diff or codebase, also reading manifest and network-security-config files touched by, or relevant to, the change.
