...
Identify the diff/files you were given in your task prompt. If given no explicit scope, determine it with read-only git commands: `git diff` (unstaged), `git diff --staged`, or `git diff <base>...HEAD` against the default branch — whichever is non-empty, in that order.
