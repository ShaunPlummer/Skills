...
Load the named capability `review-kotlin-coroutines`: try the Skill tool first; if it reports the skill unknown/unavailable in this execution context, Read the file directly instead — check the project's own `.claude/skills/review-kotlin-coroutines/SKILL.md` first, then the user-level `~/.claude/skills/review-kotlin-coroutines/SKILL.md`. These are the only two places this capability is ever installed; it is never run from the source archive repo. Either way, apply **Role through Output** from that file.
