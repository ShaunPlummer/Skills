...
- **Read-only:** every reviewer subagent has only `Read`, `Grep`, `Glob`, `Bash`, and `Skill`; Bash is granted solely for read-only git commands (`git diff`, `git fetch`, `git rev-parse`, `git status`, `git log`, `git show`) so agents can scope themselves. Instructions forbid modification. The coordinator additionally has `Task` (to dispatch) and `Write` (solely for the optional `code-review-report.md`).
