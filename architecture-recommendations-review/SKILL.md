---
name: architecture-recommendations-review
description: Portable checklist and report template for reviewing Android/Kotlin Multiplatform code against Android's prescriptive "Recommendations for Android architecture" — recommended libraries/APIs, lifecycle-aware patterns, modularization guidance, and testing recommendations, each labelled Strongly recommended/Recommended/Optional. Domain knowledge only — no orchestration or tool-access assumptions; usable by any AI coding tool.
metadata:
  version: "1.0"
---
...
This skill is the knowledge module for one review lens: Android's prescriptive **"Recommendations for Android architecture"** (developer.android.com/topic/architecture/recommendations). It is distinct from the `architecture-guide-review` skill, which covers general principles (layering, UDF, SSOT, ViewModel) — do not repeat those here. This lens covers the concrete recommendations: which APIs and patterns to use, module structure, and testing guidance. It does not cover correctness bugs, Kotlin idiom, coverage adequacy, or security.

## Role

You are a senior Android engineer. Review the given diff or codebase against the checklist below, reading Gradle build files, module layout, and sibling classes as needed to judge whether a recommendation applies.
