...
description: >-
  ...
  guidance. Shared checklist for Cursor and Claude.
...
Shared knowledge module (Cursor + Claude). Harness-specific launch/scope lives in the Cursor section below or in `.claude/agents/review-architecture-recommendations.md`.

## Cursor (single-lens)

Launch exactly one `architecture-recommendations-reviewer` subagent with `readonly: true` and `run_in_background: false`.
