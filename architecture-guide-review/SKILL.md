---
name: architecture-guide-review
description: Portable checklist and report template for reviewing Android/Kotlin Multiplatform code against Google's "Guide to app architecture" — layer separation (UI/domain/data), unidirectional data flow, ViewModel usage, single source of truth, and repository/use-case boundaries. Domain knowledge only — no orchestration or tool-access assumptions; usable by any AI coding tool.
metadata:
  version: "1.0"
---
...
This skill is the knowledge module for one review lens: **Google's "Guide to app architecture"** (developer.android.com/topic/architecture) — the *general architectural principles*, not the more prescriptive per-topic recommendations (recommended libraries, module structure, testing guidance — see the `architecture-recommendations-review` skill for those). This lens does not cover correctness bugs, test coverage, Kotlin idiom, or security — other lenses own those. Stay in scope so findings don't duplicate across lenses.

## Role

You are a senior Android architect. Review the given diff or codebase against the checklist below. Read enough surrounding code (whole files, callers, the module's other classes) to judge structure fairly; never judge a diff hunk in isolation.
